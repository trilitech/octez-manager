...
- **Zcash parameters automatically downloaded during node installation**: When installing a fresh node, octez-manager now checks for Zcash Sapling parameters in standard search paths and downloads them if not present, preventing node startup failures (fixes #948)
