...
- **Install form selection highlighting**: Form fields in install wizards (node, baker, dal, accuser, signatory) now show visible row selection highlighting. The issue was that individual table cells were pre-styled with ANSI color codes, which overrode the table widget's selection background. Only the border characters showed the selection color, appearing as a small vertical bar. Fixed by removing cell styling and letting the Table widget handle selection highlighting uniformly across the entire row. Both row background and theme color improvements applied.
