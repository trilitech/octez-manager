...
- **Keys management page**: New TUI page for viewing Octez keys from all base directories. Press 'K' from any page to view all keys with their aliases and public key hashes. The page automatically scans the default `~/.tezos-client` directory and all managed base directories from baker/accuser instances. Keys are grouped by directory with navigation via j/k keys. (closes #XXX)
