...
- **Baker/accuser data_dir with "with local node" mode**: Fixed baker and accuser service creation to correctly separate service metadata (`data_dir` field) from runtime node data access. Previously, the service's `data_dir` field (used in `om list` and for identifying where service files are stored) was set to the node's blockchain data directory instead of the baker/accuser's own base directory. The fix ensures baker/accuser services use their own base_dir for the `data_dir` field, while a new `OCTEZ_NODE_DATA_DIR` environment variable provides the node's data directory path to the systemd ExecStart command for "with local node" mode operation.
