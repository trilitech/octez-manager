...
- **Unmanaged Instances section visibility**: The "Unmanaged Instances" section (showing Octez services not managed by octez-manager) no longer intermittently disappears. Two causes were fixed: (1) template-based units (`octez-node@mainnet.service`) were incorrectly excluded by the detector because their `FragmentPath` points to the template rather than the instance — fixed by also checking for the dropin override directory; (2) a transient empty detection result (e.g. during `daemon-reload`) would clear the cache — fixed by preserving the previous cache when detection returns an empty list.
