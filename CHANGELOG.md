...
- **Signatory service installer**: Install and configure Signatory remote signer services with `om install-signatory`. Supports File backend for key storage with Memory or File watermark backends. Generates YAML configuration, creates systemd services, and manages service lifecycle. HSM and cloud KMS backends will be added in future releases. (#701)
