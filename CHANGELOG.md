...
- **Octez version display in instances list and details**: The running Octez version for each instance is now shown in the top-level instances list (visible even when folded) and in the instance details tab, making it easier to identify which version each service is running (fixes #949)
- **`--unreleased-binaries` flag**: New flag exposes prerelease binaries (release candidates, beta, alpha) when listing or resolving versions for Octez, Signatory, and octez-index. Available on the TUI launcher (`octez-manager --unreleased-binaries`) and on `om binaries list-remote`, `om binaries download octez`, and `om binaries download signatory`. Off by default — only stable releases are shown unless the flag is set. The legacy `--all/-a` flag on `list-remote` is kept as a deprecated alias. The flag is per-invocation and not persisted.
...
- **Signatory instances now launch correctly after setup**: Fixed three bugs that prevented signatory instances from starting: (1) the generated `signatory.yaml` now uses the correct `driver:` field instead of `type:` and nests file watermark paths under `config:`, matching the ECAD Labs signatory binary's expected format; (2) the install form no longer attempts to start the service twice when "enable on boot" is selected; (3) log exports now correctly identify the signatory binary version instead of looking for a non-existent `octez-signatory` binary (fixes #942)
...
- **Rewards Configuration: Min Payout / Min Balance no longer show contradictory unit**: The Configuration tab displayed values like `0.0000 tz (mutez)` for Min Payout and Min Balance — the value was already formatted as tez but tagged with `(mutez)`. The `(mutez)` suffix has been dropped; the modal prompts that take raw mutez integer input still indicate the unit.
