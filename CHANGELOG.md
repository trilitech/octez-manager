...
- **Baker multi-node support (TUI)**: Bakers can now specify extra node instances or RPC endpoints for redundancy. In the TUI baker install/edit form, the "Extra Nodes (comma-separated)" field accepts a list of managed node instances or HTTP(S) endpoints (e.g., `node2, http://backup:8732`). The baker service will use these as fallback endpoints if the primary node becomes unavailable. Extra nodes are displayed in the instance details view.
