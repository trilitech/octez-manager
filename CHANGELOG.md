...
- **Signatory config location**: Signatory YAML configuration files (`signatory.yaml`) are now stored alongside keys in `~/.local/share/octez/signatory/<instance>/` instead of split across two directories. Previously, configs were incorrectly placed in `~/.local/share/octez/instances/<instance>/` (or `~/.config/octez/instances/` briefly). This consolidates all signatory-specific files (configs + keys) in one directory, simplifying backup and management. Users with existing signatory instances should move `signatory.yaml` to the signatory data directory.
