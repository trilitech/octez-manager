...
Octez Manager is in early development. Recommended for **testnet use only** (Shadownet, Weeklynet). We do not guarantee backward compatibility with future versions.
</Aside>
...
For a PATH install with checksum verification, see the [installation guide](/getting-started/installation/).
...
<CardGrid>
	<Card title="One tool, full stack" icon="rocket">
		Node, baker, accuser, DAL node, Signatory, octez-index — all managed from a single interface.
	</Card>
...
| **Signatory** | `install-signatory` | Remote signer (HSM/cloud KMS) |
| **octez-index** | `install-index` | TzKT-compatible indexer |
