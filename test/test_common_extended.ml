...
let test_xdg_config_home_from_env () =
  ...
  let result = Paths.xdg_config_home () in
...
let test_xdg_data_home_from_env () =
  ...
  let result = Paths.xdg_data_home () in
...
let test_xdg_state_home_from_env () =
  ...
  let result = Paths.xdg_state_home () in
...
let test_exit_code_forcefully_terminated () =
  let desc = String_utils.octez_exit_code_description 255 in
  check string "forcefully terminated" "forcefully terminated" desc

let test_exit_code_config_error_range () =
  let desc1 = String_utils.octez_exit_code_description 1 in
  let desc50 = String_utils.octez_exit_code_description 50 in
  let desc125 = String_utils.octez_exit_code_description 125 in
...
let test_exit_code_shutdown_failure_range () =
  let desc129 = String_utils.octez_exit_code_description 129 in
  let desc200 = String_utils.octez_exit_code_description 200 in
  let desc253 = String_utils.octez_exit_code_description 253 in
...
let test_exit_code_unknown () =
  let desc = String_utils.octez_exit_code_description 999 in
  check
    bool
...
let test_get_editor_from_visual () =
  ...
  let result = String_utils.get_editor () in
...
let test_same_filesystem_different_paths () =
  ...
  let result = File_ops.same_filesystem "/tmp" "/var" in
  match result with
...
let test_same_filesystem_nonexistent () =
  let result = File_ops.same_filesystem "/nonexistent1" "/nonexistent2" in
...
let test_remove_path_nonexistent () =
  ...
  File_ops.remove_path "/nonexistent/file/that/doesnt/exist" ;
...
let test_default_role_dir_basic () =
  let result = Paths.default_role_dir "node" "test" in
...
let test_default_role_dir_special_chars_in_role () =
  let result = Paths.default_role_dir "bak!er@" "test" in
...
let test_default_role_dir_already_prefixed () =
  let result1 = Paths.default_role_dir "node" "node-mainnet" in
  let result2 = Paths.default_role_dir "node" "mainnet" in
...
let test_default_role_dir_empty_role () =
  let result = Paths.default_role_dir "" "test" in
...
let test_default_role_dir_whitespace () =
  let result = Paths.default_role_dir "  baker  " "  test  " in
...
let test_env_instances_base_dir () =
  let result = Paths.env_instances_base_dir () in
...
let test_default_log_dir () =
  let result = Paths.default_log_dir ~role:"node" ~instance:"test" in
  (* Should return a valid path *)
