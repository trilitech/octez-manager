...
- **`serial-tests.json`** - List of tests that must run serially (not in parallel)
  - Tests listed here run one at a time before the parallel batch
  - Used for tests that share global state (e.g., the golden path TUI test
    creates instances that interfere with parallel CLI tests)
...
GitHub Actions runs 5 jobs in parallel (one per shard), and within each shard, tests run in parallel using GNU Parallel:
...
steps:
  - run: docker compose exec -T -e PARALLEL_JOBS=3 cli-tester /run-tests.sh --shard ${{ matrix.shard }}
```

Each job:
1. Runs a subset of tests (shard) with approximately equal total duration
2. Within the shard, runs up to 3 tests in parallel simultaneously
3. This provides **two levels of parallelism**: across shards (5 jobs) and within shards (3 parallel tests)

#### Parallel Execution Within Shards

The test runner uses GNU Parallel to run multiple tests simultaneously within each shard:

```bash
# Set PARALLEL_JOBS environment variable (default: 0 = sequential)
export PARALLEL_JOBS=3

# Tests within the shard will run 3 at a time in parallel
./run-tests.sh --shard 1
```

**Benefits:**
- Reduces shard execution time by ~40-50%
- Better CPU utilization on multi-core CI runners
- Tests are still independent and isolated (each uses unique instance names)
- Maintains the same pass/fail semantics as sequential execution

**Safety:**
- Tests are designed to be independent (see AGENTS.md)
- Each test uses unique instance names and cleans up after itself
- Registry operations use file-level locking (`Unix.lockf`) in the OCaml code for concurrent access safety
- Parallel execution respects test independence requirements
