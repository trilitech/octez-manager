...
ACCUSER_INSTANCE="test-accuser-multi"
...
echo "==> Step 2: Verify all signatory services were created"
for instance in "$SIGNATORY1_INSTANCE" "$SIGNATORY2_INSTANCE" "$SIGNATORY3_INSTANCE"; do
	UNIT="signatory@${instance}.service"
	if ! systemctl list-unit-files | grep -q "$UNIT"; then
		echo "ERROR: Signatory service not found: $UNIT"
		exit 1
	fi
done

echo "==> Step 3: Verify list-signatories shows all instances"
om list-signatories 2>&1 | tee /tmp/signatory-list.txt
...
CONFIG1="/var/lib/tezos/.tezos-signatory/${SIGNATORY1_INSTANCE}/config.toml"
CONFIG2="/var/lib/tezos/.tezos-signatory/${SIGNATORY2_INSTANCE}/config.toml"
CONFIG3="/var/lib/tezos/.tezos-signatory/${SIGNATORY3_INSTANCE}/config.toml"
...
om install-baker \
	...
	--baker-key "tz2TSvNTh2epDMhZHrw73nV9piBX7kLZ9K9m" \
	--service-user tezos \
	--no-enable 2>&1

register_instance "$BAKER2_INSTANCE"

echo "==> Step 7: Install accuser using third signatory"
om install-accuser \
	--instance "$ACCUSER_INSTANCE" \
	--node-instance "$NODE_INSTANCE" \
	--protocol alpha \
	--use-signatory "$SIGNATORY3_INSTANCE" \
	--service-user tezos \
	--no-enable 2>&1

register_instance "$ACCUSER_INSTANCE"

echo "==> Step 8: Verify each baker/accuser depends on correct signatory"
...
# Accuser -> Signatory 3
if ! systemctl list-dependencies "$ACCUSER_UNIT" | grep -q "signatory@${SIGNATORY3_INSTANCE}.service"; then
	echo "ERROR: Accuser not dependent on signatory3"
	systemctl list-dependencies "$ACCUSER_UNIT"
	exit 1
fi

echo "==> Step 9: Verify cross-contamination - baker1 should NOT depend on signatory2 or signatory3"
...
echo "==> Step 10: Verify all systemd units use correct naming (signatory@, not octez-signatory@)"
for instance in "$SIGNATORY1_INSTANCE" "$SIGNATORY2_INSTANCE" "$SIGNATORY3_INSTANCE"; do
