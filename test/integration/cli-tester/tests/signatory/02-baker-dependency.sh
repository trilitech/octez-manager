...
om install-signatory \
	...
	--keys-dir "/var/lib/tezos/.tezos-signatory/keys" \
	--service-user tezos \
	--no-enable 2>&1
...
if ! systemctl list-unit-files | grep -q "signatory@${SIGNATORY_INSTANCE}.service"; then
	echo "ERROR: Signatory service unit not found"
	exit 1
fi
