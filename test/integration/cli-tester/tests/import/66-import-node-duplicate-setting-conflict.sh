...
# Verifies: config.json is preserved unchanged during import regardless of CLI args

set -euo pipefail
source /tests/lib.sh

test_init "Import node - config preserved when CLI args conflict with config.json"

# Unique instance name for this test
INSTANCE="node-dup-setting-$$"
PORT=$(alloc_port)
RPC_PORT=$(alloc_port)
...
echo "Creating external node with same setting in config.json AND CLI args..."
...
# Customize config.json with CORS and private-mode settings
CONFIG_FILE="$DATA_DIR/config.json"
jq '.rpc."cors-origin" = ["https://example.com"] |
    .p2p."private-mode" = true' \
	"$CONFIG_FILE" >"$CONFIG_FILE.tmp" && mv "$CONFIG_FILE.tmp" "$CONFIG_FILE"

echo "Config.json set with CORS origin and private-mode"
...
# Create systemd service (create_external_service adds its own CLI args)
...
echo "Verifying config.json preservation (unchanged despite CLI args in ExecStart)..."
...
echo "Test passed: Config.json preserved despite conflicting CLI args in ExecStart"
